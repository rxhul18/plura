...
export function ServiceDragableNode({ type, id, data, onDrop }: ServiceNodeProps) {
    ...
    return (
        ...
        >
            <div className={`w-full p-2 rounded-lg relative min-w-[150px] border transition-all duration-200 
                ${isDragging ? 'bg-transparent shadow-none border' : 
                isHovered ? 'bg-white border-gray-300 shadow-lg transform translate-y-[-2px]' : 
                'bg-white shadow-md border-gray-200'}`}>
                <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-center w-full dark:text-black">{data.label}</span>
                </div>
            </div>
        </div>
    );
